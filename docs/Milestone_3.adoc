...
stabilizeForm(form: FormState): FormState

correctBarPath(path: BarPath): BarPath

adjustGrip(grip: GripState): GripState
...
BreathingPatternDisrupts

Domain Functions

- respondToTightness(area: MuscleGroup): Adjustment

- manageSideStitch(stitch: Sensation): Pace

- regulateBreathing(breath: BreathingPattern): BreathingPattern
...
WorkingWeightForgotten

RepSchemeUnclear

AttentionDriftsFromExercise

AthletePausesMidRepetition

Domain Functions

recoverRepCount(state: SetState): SetState

recallPreviousLoad(log: LoadRecord): WeightLoad

reorientRepScheme(routine: RoutinePattern): RoutinePattern
...
SetEndsDueToInstability

AthleteResetsStance

Domain Functions

pauseSet(set: Set): Set

endSetEarly(set: Set): SetOutcome

resetStance(stance: Stance): Stance

adjustPace(pace: Pace): Pace

=== Load & Exercise Adjustment Patterns
Events & Behaviors

AthleteSwitchesToLighterExercise

AthleteTransitionsToMachine

WeightFeelsHeavierThanExpected

AthleteBeginsAccessoryMovement

ExerciseFlowChangesMidSession

Domain Functions

reduceLoad(load: WeightLoad): WeightLoad

switchExercise(current: Exercise): Exercise

transitionToAccessory(exercise: Exercise): Exercise
