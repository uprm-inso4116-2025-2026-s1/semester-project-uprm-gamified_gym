...
----
==== 2.1.5.1 Applying the Strategy Pattern to XP Calculation

* In GamifiedGym, experience points (XP) can be calculated in different ways depending on
the workout context—for example, XP based on repetitions, XP based on total weight
lifted, or XP based on workout duration. These represent different algorithms that may be
selected or changed over time as the gamification system evolves.

* To keep this variability manageable, the XP calculation mechanism follows the
Strategy Pattern from the lecture *Relating Design Patterns to the Model*. A common
interface (XPCalculationStrategy) defines the method for computing XP, while multiple
concrete strategies implement different formulas such as RepsBasedXPCalculation,
WeightBasedXPCalculation, or DurationBasedXPCalculation.

* This prevents the WorkoutSession aggregate from containing hard-coded conditionals
and makes the model easier to extend and maintain. By encapsulating swappable
algorithms, GamifiedGym supports future features—such as seasonal XP boosts,
difficulty modes, or exercise-specific XP rules—without changing the domain logic.
...
* Alex, the Beginner:
  ...
  ** Pain points and needs:
    ...
    certain exercises. Additionally, he prefers features that fit into busy, irregular schedules and offer clear visual rewards. He
    needs an intuitive, encouraging, and gamified guide to help him build a foundation.
...
==== 2.2.6 Modeling Entities
* User
** Role: Core entity representing the app user (gym-goer).
** Responsibilities: Stores personal data, progress, completed workouts, challenges, achievements, rewards, level, and streaks.

* Workout Session
** Role: Represents a single gym session logged by the User.
** Responsibilities: Contains exercises, tracks metrics like duration, calories burned, and triggers achievement checks.

* Exercise
** Role: Atomic unit of a Workout Session.
** Responsibilities: Tracks exercise-specific stats (sets, reps, duration, calories). Contributes to Goals.

* Goal
** Role: Represents user-defined or system-defined targets.
** Responsibilities: Tracks progress (frequency, consistency, performance). Completion linked to Achievements and Rewards.

* Achievement
** Role: Gamification reward for reaching milestones.
** Responsibilities: Recognizes completion of Goals, streaks, or Challenges. May grant Rewards or badges.

* Reward
** Role: Incentives to motivate Users.
** Responsibilities: Can be points, badges, in-app currency, or unlockables. Linked to Achievements, Levels, or Challenge completion.

* Level
** Role: Represents progression in gamification.
** Responsibilities: Tracks XP accumulation and unlocks higher-level features. Each User has a Level.

* Challenge
** Role: Social or individual competitions.
** Responsibilities: Tracks participating Users, goals, or exercises for completion. Grants Achievements/Rewards upon success.

* Leaderboard
** Role: Provides a ranked view of Users based on XP, points, or streaks.
** Responsibilities: Aggregates User stats. Typically read-only; updated dynamically.

* Scalability Notes
** New entities like Badges, Community Events, or Friend interactions can be added by linking them with User or Challenge.
** Many-to-many relationships allow social and competitive features without altering the core model.
** Composition (Workout Session → Exercise) ensures exercises are always tied to a session, maintaining data integrity.

==== 2.2.7 Acceptance Criteria, Tests, and Done Definition

* Done definition (feature-level)
** Workout Logging: User can start a session, add exercises and sets, tap "Done", see a confirmation (toast/alert), and observe the persisted session in the Saved Workouts history.
** Saved Workouts Management: Saved workouts appear in history, can be opened, edited (title/details), and deleted; edits persist and UI updates immediately.
** XP & Achievements: Completing a saved workout results in XP being calculated and stored; unlocking achievements triggers a visible UI notification.
** Data Integrity: All created/edited/deleted workouts are persisted in Supabase and survive app restart.

* Automated tests
** Unit tests
*** XPCalculationStrategy: verify outputs for RepsBased, WeightBased, DurationBased strategies for a variety of inputs.
*** WorkoutSession invariants: tests for non-negative duration and positive reps/weights.
*** Small pure helpers (e.g., volume calculation) are side-effect free and fully unit-tested.
** Integration tests
*** WorkoutRepository <> Supabase: create/read/update/delete roundtrip tests (mock Supabase in CI).
*** UI component integration: ExerciseCard + ExerciseLog interaction test, verifying onDone callback triggers toast/alert (use jest + react-native-testing-library).
** End-to-end tests
*** E2E scenario: start workout → add push-up set → tap Done → assert "Workout Saved Successfully." toast/alert shown → open Saved Workouts → workout present. (use Detox or Appium)

* Example acceptance test cases (concise)
1. Log a push-up and tap Done:
   - Given the user is on the workout screen
   - When the user completes the exercise and taps Done
   - Then a confirmation "Workout Saved Successfully." is shown and the workout appears in Saved Workouts
2. Edit a saved workout title:
   - Open history entry, edit title, save
   - Title updates in list and persists after app restart
3. Delete a saved workout:
   - Delete confirmation shown; upon confirmation the item is removed and backend record deleted
4. XP calculation correctness:
   - Given a predefined set of sets/reps/weight, the selected XP strategy returns expected XP value

==== 2.2.8 Traceability to Evaluation Guidelines & Applied Patterns

* Specification pattern
** Usage: Filters and search (e.g., select saved workouts by date range, muscle group, or min-volume) implemented as Specification objects under src/domain/specifications.
** Test evidence: unit tests for each specification; integration tests for composition of specs.

* Strategy pattern
** Usage: XP calculation is pluggable via XPCalculationStrategy implementations (RepsBased, WeightBased, DurationBased) located under src/domain/xp/strategies.
** Test evidence: unit tests for each strategy and a factory/injector that selects the strategy per workout type.

* Intention-revealing interfaces
** Approach: Public APIs expose high-level intent (e.g., WorkoutRepository.saveWorkout, XPCalculationStrategy.calculateXP) rather than transport/storage details.
** Refactor notes: interfaces were renamed to clarify intent where originally names were ambiguous.

* Side-effect free functions & command-query separation
** Examples: Volume calculation and XP pure helpers are side-effect free and fully unit-tested.
** Command-query separation: methods that mutate state are separated (e.g., saveWorkout) from pure query methods (e.g., getWorkoutSummary).

* Assertions
** Examples: Precondition checks in domain commands (e.g., assert sets.length > 0 before completing workout) documented and unit-tested.

* Stand-alone classes & closure under operations
** Examples: WorkoutRepository is designed to be replaceable and testable without UI dependencies; XP strategies are closed under operations required for XP computation.

* Other patterns (composite, repository, factory)
** Composite: Exercise entries composed into WorkoutSession aggregate.
** Repository: WorkoutRepository abstracts persistence (src/repositories/WorkoutRepository.ts).
** Factory: AchievementFactory and XPCalculationStrategy factory for creating instances with complex initialization.

==== 2.2.9 Acceptance Test Matrix (summary)

* Feature → Key test → Automation level
** Workout Logging → "Done" creates persisted workout + toast → E2E + Integration
** Saved Workouts CRUD → edit/delete persist and reflect in UI → Integration + Unit
** XP Calculation → correct XP per strategy → Unit
** Achievements → unlocked when criteria met → Integration/E2E
** Supabase persistence → CRUD roundtrip → Integration (mocked CI) / manual verification in staging

==== 2.2.10 Implementation Notes & File Locations (where to look)

* UI
** app/(tabs)/ExerciseLog.tsx — Saved Workouts list, modal for view/edit/delete
** components/exercise-card.tsx — exercise entry UI and "Done" action (should call onDone callback)
* Domain
** src/domain/xp/strategies — XP calculation strategies
** src/domain/specifications — specification pattern implementations for filtering/selection
* Persistence
** src/repositories/WorkoutRepository.ts — create/read/update/delete workout persistence to Supabase
* Tests
** __tests__/unit — unit tests for strategies and pure functions
** __tests__/integration — repository and Supabase integration tests (mocked)
** e2e/ — Detox scenarios for key end-to-end flows

[NOTE]
====
- CHANGED: Added Saved Workouts history UI (view / edit / delete) and onDone toast confirmation (app/(tabs)/ExerciseLog.tsx, components/exercise-card.tsx).
- CHANGED: Expanded XP strategy descriptions and acceptance criteria for XP/achievements.
- CHANGED: Added Acceptance Test Matrix and Implementation Notes pointing to repository and test locations.
- CHANGED: Defined modeling entities and acceptance criteria in 2.2.6–2.2.10.
- How to find edits: each paragraph or bullet that was modified in this submission begins with the token "CHANGED:" for quick review.
====
