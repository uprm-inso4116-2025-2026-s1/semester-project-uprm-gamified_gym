...
==== 2.1.4 Narrative
This domain revolves around the athlete stepping into the gym and performing workout routines designed to 
strengthen their body and improve their endurance. Each session unfolds as a sequence of sets and repetitions, 
targeting specific muscle groups such as chest, back, legs, arms, shoulders, etc. The athlete measures 
progress not only by the number of weights lifted or repetitions completed, but also by changes in physical 
in their appearance and overall stamina.

As the workouts continues, the athlete faces the familiar challenge of consistency. Some days the motivation 
is high, fueled by personal goals; other days, fatigue or lazyness makes them lack motivation. Without 
proper training, certain muscle groups may be trained more than others, leading to overtraining. 
The athlete attempts to track routines through notebooks, mental notes, or apps, yet these records often 
lack meaningful feedback, leaving gaps in awareness of progress and balance.

Within the rhythm of gym culture, motivation is sustained through competition, social comparison, and 
goals. The athlete celebrates victories (a heavier lift, an extra repetition, or visible physical 
change) and finds encouragement in comparing progress with peers. Achievements are marked by badges, 
challenges, and symbols of recognition, whether a weightlifting belt, a gym award, or acknowledgment 
within a community.

The flow of this domain is not limited to the physical act of exercising. It encompasses the athlete’s 
drive for self‑improvement, the ongoing effort to track performance, the need to maintain balance across 
training, and the search for motivation through challenges and recognition.
...
function fixGrip(grip: Grip): Grip

function steadyMovement(movement: Movement): Movement

function correctBarPath(path: BarPath): BarPath

function adjustFootStrike(strike: FootStrike): FootStrike

function slowDownBecauseOfTightness(area: MuscleArea): Pace

function pauseBecauseOfStitch(stitch: SideStitch): Pace

function adjustBreathing(breathing: BreathingPattern): BreathingPattern

function recoverRepCount(set: Set): Set

function recallPreviousWeight(exercise: Exercise): WeightLoad

function recoverRepScheme(scheme: RepScheme): RepScheme

function refocusOnExercise(exercise: Exercise): Exercise

function pauseSet(set: Set): Set

function endSetEarly(set: Set): Set
...
function switchToLighterExercise(exercise: Exercise): Exercise

function transitionToAccessoryExercise(exercise: Exercise): Exercise
